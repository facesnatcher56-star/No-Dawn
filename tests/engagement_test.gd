...
func run() -> void:
	...
	check(absf(game.sim_time - 10) < 0.001, "Execution advances five seconds per tank")
	...
	game._execute()
	check(await Harness.finish_turn(game), "Execution and all impact replays finish")
	check((absf(game.sim_time - 20) < 0.001 or (game.phase == "COMPLETE" and absf(game.sim_time - 15) < 0.001)), "Second sequential execution completes")
