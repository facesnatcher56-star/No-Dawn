...
func run() -> void:
	...
	check(game.phase in ["ASSESSMENT", "COMPLETE"], "Simultaneous execution ends in assessment")
	print("Sim_time after turn 1: ", game.sim_time)
	check(game.sim_time >= 2.9, "Simultaneous execution advances timeline")
	...
	check(game.display_contact.source in ["Visual silhouette", "Commander optics", "Briefing / unconfirmed"], "A scan retains its useful last-seen fix instead of replacing it with weaker sound estimates")
	game.fields.fire.button_pressed = true
	game._execute()
	check(await Harness.finish_turn(game), "Execution and all impact replays finish")
	check(game.phase in ["ASSESSMENT", "COMPLETE"], "Second simultaneous execution completes")
