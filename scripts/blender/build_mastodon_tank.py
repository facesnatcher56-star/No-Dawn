...
def build_mastodon():
    ...
    # -------------------------------------------------------------
    # Upper Glacis (90mm, 55 deg slope)
    arm_ug = create_box(
        "ARM_UpperGlacis", (2.12, 1.45, 0.09), (0.0, 2.55, 1.42),
    ...
    arm_lg = create_box(
        "ARM_LowerGlacis", (2.12, 1.05, 0.075), (0.0, 3.25, 0.78),
    ...
    arm_rear = create_box(
        "ARM_HullRear", (2.12, 1.25, 0.055), (0.0, -3.42, 1.25),
    ...
    arm_floor = create_box(
        "ARM_HullFloor", (2.12, 5.80, 0.025), (0.0, -0.15, 0.44),
        material=mats["MAT_Armor_Exterior"], parent=hull, collection=col_armor
    )
    ...
    # -------------------------------------------------------------
    # 3. INTERNAL COMPARTMENTS & MECHANICAL COMPONENTS (CMP_*)
    # -------------------------------------------------------------
    # Front Transmission (between Driver and Radio Operator)
    cmp_trans = create_box(
        "CMP_Transmission", (0.92, 0.85, 0.65), (0.0, 2.75, 0.78),
    ...
    # Driveshaft (Engine -> Transmission through lower hull)
    ...
    # Firewall (Between Fighting Compartment & Engine)
    cmp_firewall = create_box(
        "CMP_Firewall", (2.08, 0.04, 1.48), (0.0, -1.25, 1.20),
        material=mats["MAT_Steel_Cast"], parent=hull, collection=col_components
    )
    
    # Engine (V12 Alternate-History Heavy Tank Engine)
    cmp_engine = create_box(
        "CMP_Engine", (1.10, 1.70, 0.75), (0.0, -2.25, 0.95),
        material=mats["MAT_Engine_Block"], parent=hull, collection=col_components
    )
    # V12 Cylinder Heads & Valve Covers
    for side in [-1, 1]:
        create_box(
            f"VIS_EngineBank_{'L' if side < 0 else 'R'}", (0.34, 1.55, 0.28),
            (side * 0.35, -2.25, 1.38), rotation=(0, side * math.radians(30), 0),
            material=mats["MAT_Engine_Red"], parent=hull, collection=col_components
        )
        # Dual Exhaust Manifolds
        create_cylinder(
            f"VIS_ExhaustManifold_{'L' if side < 0 else 'R'}", 0.055, 1.65,
            (side * 0.58, -2.25, 1.15), rotation=(math.pi / 2, 0, 0),
            material=mats["MAT_Gunmetal"], parent=hull, collection=col_components
        )
    # Dual Air Cleaner / Carburetor Units
    create_cylinder(
        "VIS_AirCleaner_L", 0.16, 0.30, (-0.22, -1.65, 1.65),
        material=mats["MAT_Gunmetal"], parent=hull, collection=col_components
    )
    create_cylinder(
        "VIS_AirCleaner_R", 0.16, 0.30, (0.22, -1.65, 1.65),
        material=mats["MAT_Gunmetal"], parent=hull, collection=col_components
    )
    ...
    # -------------------------------------------------------------
    # 4. HULL AMMUNITION STORAGE (AMMO_*)
    # -------------------------------------------------------------
    ...
    # -------------------------------------------------------------
    # 5. HULL CREW (CREW_Driver, CREW_RadioOperator)
    # -------------------------------------------------------------
    ...
    # Driver Controls (steering tillers, instrument binnacle)
    ...
    # Hull Machine Gun
    create_cylinder(
        "CMP_HullMG", 0.04, 0.45, (0.55, 2.92, 1.45), rotation=(math.pi / 2, 0, 0),
        material=mats["MAT_Gunmetal"], parent=hull, collection=col_exterior
    )
    create_empty("MKR_HullMG_Muzzle", (0.55, 3.15, 1.45), parent=hull, collection=col_markers)
    ...
    for side_idx, (side, side_str) in enumerate([(-1, "L"), (1, "R")]):
        ...
        for w_idx, y_pos in enumerate(wheel_y_positions):
            ...
            create_box(
                f"VIS_SuspArm_{side_str}_{w_idx+1:02d}", (0.08, 0.28, 0.08),
        ...
        sprocket = create_sprocket_wheel(
            f"CMP_DriveSprocket_{side_str}", (side * 1.36, 3.10, 0.65), radius=0.38, width=0.24,
            parent=hull, collection=col_running_gear, steel_mat=mats["MAT_Steel_Cast"]
        )
        
        # Idler (Rear)
        idler = create_road_wheel(
        ...
        for r_idx, r_y in enumerate([-1.85, -0.60, 0.65, 1.90]):
            roller = create_road_wheel(
                f"CMP_Roller_{side_str}_{r_idx+1:02d}", (side * 1.36, r_y, 1.05), radius=0.14, width=0.18,
                parent=hull, collection=col_running_gear, steel_mat=mats["MAT_Steel_Cast"]
            )

        # Track Bands (Continuous run covering wheels, idler, sprocket)
        # Bottom run
        create_box(
            f"CMP_Track_{side_str}_Bottom", (0.58, 5.20, 0.04), (side * 1.36, -0.05, 0.09),
            material=mats["MAT_Track_Steel"], parent=hull, collection=col_running_gear
        )
        # Top run
        create_box(
            f"CMP_Track_{side_str}_Top", (0.58, 5.20, 0.04), (side * 1.36, -0.05, 1.20),
            material=mats["MAT_Track_Steel"], parent=hull, collection=col_running_gear
        )
        # Front slant to sprocket
        create_box(
            f"CMP_Track_{side_str}_Front", (0.58, 1.15, 0.04), (side * 1.36, 2.80, 0.65),
            rotation=(-math.radians(35), 0, 0),
            material=mats["MAT_Track_Steel"], parent=hull, collection=col_running_gear
        )
        # Rear slant to idler
        create_box(
            f"CMP_Track_{side_str}_Rear", (0.58, 1.15, 0.04), (side * 1.36, -2.85, 0.65),
            rotation=(math.radians(35), 0, 0),
            material=mats["MAT_Track_Steel"], parent=hull, collection=col_running_gear
        )
        
        # Sponson mudguards / track covers
        create_box(
            f"VIS_TrackFender_{side_str}", (0.64, 6.80, 0.03), (side * 1.36, 0.0, 1.26),
    ...
    # Turret Body Base Structure
    turret_body = create_box(
        "Turret", (2.42, 3.20, 0.88), (0.0, 0.05, 0.48),
        material=mats["MAT_Armor_Exterior"], parent=turret_pivot, collection=col_exterior
    )
    ...
    # Commander Cupola (80mm cast armor ring)
    arm_cupola = create_cylinder(
        "ARM_CommanderCupola", 0.42, 0.28, (0.62, -0.45, 1.08),
        material=mats["MAT_Steel_Cast"], parent=turret_pivot, collection=col_armor
    )
    ...
    # Cupola Vision Blocks (6 periscopes)
    ...
    # Ready Ammo Rack (Turret bustle, 12 rounds)
    create_ammo_rack(
    ...
    # -------------------------------------------------------------
    # 8. TURRET CREW (Gunner, Commander, Loader)
    # -------------------------------------------------------------
    ...
    # -------------------------------------------------------------
    # 9. MAIN ARMAMENT & ELEVATION ASSEMBLY
    # -------------------------------------------------------------
    ...
    # Cast Gun Mantlet (120mm armor)
    arm_mantlet = create_cylinder(
        "ARM_GunMantlet", 0.48, 0.82, (0.0, 0.45, 0.0),
        rotation=(0, math.pi / 2, 0),
        material=mats["MAT_Steel_Cast"], parent=gun_assembly, collection=col_armor
    )
    ...
    # Gun Barrel (92mm L/58 Cannon: 4.6m exposed length)
    # Base sleeve
    create_cylinder(
        "VIS_BarrelReinforcement", 0.125, 1.20, (0.0, 1.15, 0.0),
        rotation=(math.pi / 2, 0, 0),
    ...
    # Coaxial Machine Gun (7.92mm / .30 cal)
    create_cylinder(
    ...
    # -------------------------------------------------------------
    # 10. EXTERNAL STOWAGE, TOOLS, EXHAUST, DETAILS
    # -------------------------------------------------------------
    # Armored Engine Deck Louvers / Grilles
    ...
    # Pioneer Tools (Shovel, Axe, Pickaxe)
    create_box(
    ...
    # Tow Cables along sponson edges
    for side in [-1, 1]:
        create_cylinder(
    ...
    # Heavy Tow Hooks / Cleats (Front & Rear)
    for fx in [-0.85, 0.85]:
        create_box(
            f"VIS_TowHookFront_{'L' if fx < 0 else 'R'}", (0.10, 0.24, 0.16), (fx, 3.62, 0.72),
            material=mats["MAT_Steel_Cast"], parent=hull, collection=col_exterior
        )
        create_box(
            f"VIS_TowHookRear_{'L' if fx < 0 else 'R'}", (0.10, 0.24, 0.16), (fx, -3.55, 0.72),
            material=mats["MAT_Steel_Cast"], parent=hull, collection=col_exterior
        )

    # Turret Lifting Eyes (4 corners)
    ...
    # Headlight with blackout cover (Front Left fender)
    create_cylinder(
        "VIS_Headlight_L", 0.10, 0.14, (-1.35, 3.35, 1.38),
        rotation=(math.pi / 2, 0, 0),
        material=mats["MAT_Gunmetal"], parent=hull, collection=col_exterior
    )
