...
func _init(ammunition: int = 25) -> void:
	...
	_plate("Glacis", Vector3(0, 1.25, -2.9), Vector3(3.4, 1.85, 0.07), 70.0, deg_to_rad(-18))
	_plate("Rear", Vector3(0, 1.25, 2.9), Vector3(3.4, 1.85, 0.04), 40.0)
	_plate("Left hull", Vector3(-1.7, 1.25, 0), Vector3(0.04, 1.85, 6.4), 40.0)
	_plate("Right hull", Vector3(1.7, 1.25, 0), Vector3(0.04, 1.85, 6.4), 40.0)
	_plate("Deck", Vector3(0, 2.16, 0), Vector3(3.4, 0.02, 6.4), 20.0)
	_plate("Floor", Vector3(0, 0.33, 0), Vector3(3.4, 0.02, 6.4), 20.0)
	_plate("Turret front", Vector3(0, 2.65, -1.25), Vector3(2.5, 1, 0.08), 80.0, 0, true)
	_plate("Turret rear", Vector3(0, 2.65, 1.25), Vector3(2.5, 1, 0.04), 40.0, 0, true)
	_plate("Turret left", Vector3(-1.25, 2.65, 0), Vector3(0.05, 1, 2.5), 50.0, 0, true)
	_plate("Turret right", Vector3(1.25, 2.65, 0), Vector3(0.05, 1, 2.5), 50.0, 0, true)
	_plate("Turret roof", Vector3(0, 3.15, 0), Vector3(2.5, 0.02, 2.5), 20.0, 0, true)
...
func _plate(label: String, pos: Vector3, size: Vector3, mm: float, slope = 0.0, turret = false) -> void:
	plates.append({"name": label, "pose": Transform3D(Basis(Vector3.RIGHT, slope), pos), "size": size, "mm": mm, "turret": turret})
...
func _trace(origin: Vector3, dir: Vector3, energy: float, fragment: bool, result: Dictionary, rng: RandomNumberGenerator) -> void:
	...
	for hit in hits:
		...
		if v.has("mm"):
			...
			var cost: float = effective / 140.0 * (0.5 * MASS * MUZZLE_SPEED * MUZZLE_SPEED)
			var angle = rad_to_deg(acos(clampf(cosine, 0, 1)))
			var outcome = "PENETRATION" if energy > cost else "STOPPED"
			if angle > 72 and not fragment: outcome = "RICOCHET"
			if not fragment:
				result.impacts.append({"position": point, "normal": hit.normal, "plate": v.name, "mm": v.mm, "effective": effective, "angle": angle, "energy_in": energy, "outcome": outcome})
			...
			if fragment or outcome != "PENETRATION":
				...
				if outcome == "RICOCHET" and not fragment: result.paths.append({"from": point, "to": point + dir.bounce(hit.normal) * 4, "fragment": false})
				return
			energy -= cost
			if not fragment:
				result.impacts.back()["energy_out"] = energy
				result.impacts.back()["residual_speed"] = sqrt(2 * energy / MASS)
			if not entered:
				...
				for i in range(18):
					var spread = Vector3(rng.randf_range(-0.38, 0.38), rng.randf_range(-0.38, 0.38), rng.randf_range(-0.38, 0.38))
					_trace(inside, (dir + spread).normalized(), fragment_energy / 18, true, result, rng)
			entered = true
		elif entered:
